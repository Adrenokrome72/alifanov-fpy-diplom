...
class StorageTests(TestCase):
    ...
    def test_create_folder(self):
        response = self.client.post('/api/storage/folders/create/', {'name': 'newfolder'})  # Omit parent
    ...
    def test_download_file(self):
        file = File.objects.create(original_name='test.txt', unique_name=str(uuid.uuid4()), size=12, owner=self.user)
        path = os.path.join(settings.MEDIA_ROOT, str(self.user.id), str(file.unique_name))
    ...
    def test_share_file(self):
        ...
        file = File.objects.create(original_name='test.txt', unique_name=str(uuid.uuid4()), size=12, owner=self.user, share_link=share_uuid)
        path = os.path.join(settings.MEDIA_ROOT, str(self.user.id), str(file.unique_name))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, 'wb') as f:
            f.write(b"content")
        response = self.client.get(f'/api/storage/share/{share_uuid}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_storage_limit(self):
        self.user.storage_limit = 10
        self.user.save()
        file = SimpleUploadedFile("large.txt", b"content_over_limit", content_type="text/plain")
