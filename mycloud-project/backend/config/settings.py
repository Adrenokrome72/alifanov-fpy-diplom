...
CORS_ALLOWED_ORIGINS = [
    "http://localhost:3000",
    "http://127.0.0.1:3000",
] + [f"http://{host.strip()}" for host in os.getenv("CORS_ALLOWED_ORIGINS", "").split(",") if host.strip() and not host.strip().startswith("http://")]
...
CSRF_TRUSTED_ORIGINS = [
    "http://localhost:3000",
    "http://127.0.0.1:3000",
] + [f"http://{host.strip()}" for host in os.getenv("CSRF_TRUSTED_ORIGINS", "").split(",") if host.strip() and not host.strip().startswith("http://")]
