...
**Cloud** - это веб-приложение, которое работает как облачное хранилище. Приложение позволяет пользователям загружать, скачивать, просматривать и переименовывать файлы, а также управлять ими через удобный веб-интерфейс.
...
- ✅ Генерация специальных ссылок для внешнего доступа к файлам (Поделиться)
- ✅ Просмотр статистики загрузок и дат последнего скачивания
- ✅ Перемещение файлов внутри хранилища
...
- **Django REST Framework 3.16.1** - для API
- **Django CORS Headers 4.9.0** - для обработки CORS

#### Frontend:
- **React 19.2** - библиотека для создания пользовательского интерфейса
...
- **Docker 28.5.1** - Контейниризация
- **Docker compose 2.40.3** - Управление многоконтейнерными приложениями
- **Nginx 1.29.3** - веб-сервер
## Установка и запуск

### 1. Клонирование репозитория

```bash
git clone https://github.com/Adrenokrome72/alifanov-fpy-diplom
cd mycloud-project
```
...
1. Нажмите на файл левой кнопкой мыши
2. Выберите "Переименовать" в появившемся окне
3. Введите новое имя и нажмите "Сохранить"

#### Удаление файла:
1. Нажмите на файл левой кнопкой мыши
2. Выберите "Удалить" в появившемся окне
...
3. Администратору доступен только просмотр содержимого папок и просмотр файлов.
